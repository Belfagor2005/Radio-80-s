...
class radiom80(Screen):
    def __init__(self, session, name, url, pic):
        ...
        self['key_red'] = Button(_('Exit'))
        self['key_green'] = Button(_('Player 1'))
        self.player = '1'
    ...
    def typeplayer(self):
        if self.player == '1':
            self["key_green"].setText(_("Player 2"))
            self.player = '2'
        else:
            self["key_green"].setText(_("Player 1"))
            self.player = '1'
    ...
    def loadPlaylist(self):
        ...
        if a == 0:
            ...
            res = Utils.ReadUrl2(self.url, referer)
            if six.PY3:
    ...
    def titlesong(self, urla):
        try:
            ...
            r = http.get(urla, headers=hdr, timeout=10, verify=False, stream=True)
            r.raise_for_status()
